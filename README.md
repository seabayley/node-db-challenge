...
- [X] Explain the difference between `Relational Databases` and `SQL`.
 A relational database in a collection of data stored in tablets in a way that is easily accessed and manipulated. SQL is query language used to manage and manipulate that data.

- [X] Why do tables need a `primary key`?
Tables need a primary key so that they can be uniquely identified by queries.

- [X] What is the name given to a table column that references the primary key on another table.
A foreign key.

- [X] What do we need in order to have a _many to many_ relationship between two tables.
A third table in which you set up foreign key references to the initial two tables.
